...
class App {
  ...
  setupEventHandlers() {
    ...
    ipcMain.handle('check-gemini-ready', this.handleCheckGeminiReady.bind(this));
    ipcMain.handle('check-latex-installation', this.handleCheckLatexInstallation.bind(this));
    ...
    ipcMain.handle('generate-design-document', this.handleGenerateDesignDocument.bind(this));
    ipcMain.handle('convert-latex-to-pdf', this.handleConvertLatexToPdf.bind(this));
    ...
    ipcMain.handle('get-design-documents', this.handleGetDesignDocuments.bind(this));
    ...
    ipcMain.handle('check-ollama-ready', this.handleCheckOllamaReady.bind(this));
    ipcMain.handle('generate-design-document-ollama', this.handleGenerateDesignDocumentOllama.bind(this));
  }
  ...
  initializeHandlers() {
    ...
    try {
      ...
      console.log('Gemini handler initialized successfully for comprehensive documents');
    } catch (error) {
  ...
  async handleCheckOCRReady() {
    return {
      ...
      error: !this.ocrHandler ? 'OCR handler not initialized. Check your MISTRAL_API_KEY.' : null
    };
  }
  ...
  async handleCheckGeminiReady() {
    return {
      ...
      error: !this.geminiHandler ? 'Gemini handler not initialized. Check your GEMINI_API_KEY.' : null
    };
  }
  ...
  async handleCheckLatexInstallation() {
    ...
    return {
      installed,
      error: !installed ? 'LaTeX not found. Please install TeX Live, MiKTeX, or another LaTeX distribution.' : null
    };
  }
  ...
  async handleGenerateDesignDocumentOllama(event, ocrData, filename) {
    try {
      ...
      if (result.success) {
        const saveResult = await this.localLLMHandler.saveDesignDocument(result.latex, filename);
        return {
          success: true,
          latex: result.latex,
  ...
  async handleSelectPdfFile() {
    try {
      const result = await dialog.showOpenDialog(this.mainWindow, {
        ...
        defaultPath: process.platform === 'linux' ? require('os').homedir() : undefined
      });
  ...
  async handleProcessPdfOCR(event, filePath) {
    try {
      ...
      const result = await this.ocrHandler.processWithBase64(base64Data, filename);

      console.log(`OCR completed. Text length: ${result.extractedText?.length || 0} characters`);
  ...
  async handleGenerateDesignDocument(event, ocrData, filename) {
    try {
      ...
      console.log(`Input data size: ${JSON.stringify(ocrData).length} characters`);

      const result = await this.geminiHandler.generateDesignDocument(ocrData);

      if (result.success) {
        // Save the LaTeX document
        const saveResult = await this.geminiHandler.saveDesignDocument(result.latex, filename);

        console.log(`Design document generated: ${result.latex.length} characters`);
  ...
  async handleConvertLatexToPdf(event, latexFilePath) {
    try {
      ...
      const validation = await this.latexConverter.validateLatexSyntax(latexFilePath);
  ...
  async handleGetDesignDocuments() {
    try {
      ...
      const documentFiles = files.filter(file => file.endsWith('.tex') || file.endsWith('.pdf'));
